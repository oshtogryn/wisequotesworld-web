...
const RULES=[
 ...
 ['social_copy','social_to_local_article','Every social post is self-contained and substantive and funnels to the exact same-language article when the platform supports a useful clickable destination. Never send a locale to the homepage or another language when a localized article exists.'],
 ['social_copy','platform_link_policy','Facebook and Threads copy must include the exact clickable same-language article URL. Instagram, TikTok and YouTube video descriptions must not include the raw article URL because it is not a useful clickable destination there; use a natural localized CTA telling the viewer that the link is in the profile.'],
...
async function applyConfirmedOperationalState(env){
  ...
  // User confirmed the 2026-08-30 WQ011 posts were published; website pages were already live.
