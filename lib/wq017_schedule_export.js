...
async function markWQ019Scheduled(env){
  ...
  try{await env.DB.exec(`CREATE TABLE IF NOT EXISTS editorial_status_history(id INTEGER PRIMARY KEY AUTOINCREMENT,project_id TEXT NOT NULL,content_item_id TEXT NOT NULL,from_status TEXT,to_status TEXT NOT NULL,notes TEXT,actor TEXT,created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`);await env.DB.prepare(`INSERT INTO editorial_status_history(project_id,content_item_id,from_status,to_status,notes,actor,created_at) VALUES('wisequotesworld','WQ019',?,'scheduled','Metricool Planner readback confirmed 64/64 active non-draft posts on 2026-09-07','automation',?)`).bind(before?.status||null,ts).run()}catch{}
  ...
  return {ok:after?.status==='scheduled',before,after};
}
export async function wq017ScheduleExport(request,env){
  ...
  try{
    ...
    if(request.method==='POST'&&url.pathname==='/ops/run/wq019-scheduled-64-v1'&&request.headers.get('x-wq019-schedule')==='wq019-64-confirmed-20260907')return json(await markWQ019Scheduled(env));
    return null;
