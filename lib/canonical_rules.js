...
const RULES=[
 ...
 ['social_copy','social_to_local_article','Every social post is self-contained and substantive and funnels to the exact same-language article URL when one exists. Never send a locale to the homepage or another language when a localized article exists.'],
...
async function applyConfirmedOperationalState(env){
  ...
  // Metricool Planner readback on 2026-08-30 confirmed 8/8 locales, each with FB + IG + Threads x3 + TikTok + YouTube + Pinterest.
  await env.DB.prepare(`UPDATE content_items SET status='scheduled' WHERE project_id=? AND id='WQ011' AND status<>'published'`).bind(PROJECT_ID).run();
  await env.DB.prepare(`UPDATE content_items SET status='published' WHERE project_id=? AND id='WQ006'`).bind(PROJECT_ID).run();
  await env.DB.prepare(`UPDATE quote_pages SET status='published',published_at=COALESCE(published_at,'2026-08-29'),updated_at=? WHERE project_id=? AND content_item_id='WQ006'`).bind(ts,PROJECT_ID).run();
}
...
export async function syncCanonicalRules(env){
 ...
 for(const [group,key,value] of RULES){
   ...
   if(existing?.id){
     await env.DB.prepare(`UPDATE rules SET rule_group=?,rule_value=?,notes='Canonical runtime rule synchronized from code/MASTER_RULES v4.5 + latest explicit decisions.',mandatory=1,status='approved',effective_from=? WHERE id=?`).bind(group,value,t,existing.id).run();
   }else{
     await env.DB.prepare(`INSERT INTO rules(project_id,scope_type,language_code,platform_code,rule_group,rule_key,rule_value,notes,mandatory,status,version,effective_from) VALUES(?, 'project', NULL, NULL, ?, ?, ?, 'Canonical runtime rule synchronized from code/MASTER_RULES v4.5 + latest explicit decisions.', 1, 'approved', 1, ?)`).bind(PROJECT_ID,group,key,value,t).run();
   }
 }
...
export async function readCanonicalRules(env){
 ...
 return {ok:true,expected:keys.length,present:keys.filter(k=>latest[k]).length,missing:keys.filter(k=>!latest[k]),rules:keys.map(k=>latest[k]||{rule_key:k,missing:true}),operational:{items,wq006_pages:wq006Pages}};
}
