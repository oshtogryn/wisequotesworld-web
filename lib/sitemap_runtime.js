...
import {PROJECT_ID,WEBSITE_LOCALES} from './project_config.js';

const BASE='https://wisequotesworld.com';
const LOCALES=WEBSITE_LOCALES;
const LOCALE_SQL=LOCALES.map(x=>`'${x.replace(/'/g,"''")}'`).join(',');
...
function visibleSql(alias='ci'){return `(COALESCE(${alias}.sequence_no,CAST(SUBSTR(${alias}.id,3) AS INTEGER),0)<=15 OR EXISTS (SELECT 1 FROM content_approvals sva WHERE sva.content_item_id=${alias}.id AND sva.approval_scope='website_visibility' AND sva.language_code IS NULL AND sva.status='approved') OR EXISTS (SELECT 1 FROM website_publication_schedule ws WHERE ws.content_item_id=${alias}.id AND ws.project_id='${PROJECT_ID}' AND ws.status='scheduled' AND ws.scheduled_for<=strftime('%Y-%m-%dT%H:%M:%fZ','now')))`}
...
async function quoteRows(env){
  return (await env.DB.prepare(`SELECT qp.content_item_id,qp.language_code,qp.canonical_path,qp.updated_at,qp.published_at FROM quote_pages qp WHERE qp.project_id=? AND qp.status='published' AND qp.language_code IN (${LOCALE_SQL}) AND TRIM(COALESCE(qp.canonical_path,''))<>'' ORDER BY qp.content_item_id,qp.language_code`).bind(PROJECT_ID).all()).results||[];
}
async function categoryRows(env){
  return (await env.DB.prepare(`SELECT DISTINCT qp.language_code,ci.category_slug FROM quote_pages qp JOIN content_items ci ON ci.id=qp.content_item_id WHERE qp.project_id=? AND qp.status='published' AND ${visibleSql('ci')} AND qp.language_code IN (${LOCALE_SQL}) AND TRIM(COALESCE(ci.category_slug,''))<>'' ORDER BY qp.language_code,ci.category_slug`).bind(PROJECT_ID).all()).results||[];
}
async function authorRows(env){
  return (await env.DB.prepare(`SELECT DISTINCT qp.language_code,ci.author_name FROM quote_pages qp JOIN content_items ci ON ci.id=qp.content_item_id WHERE qp.project_id=? AND qp.status='published' AND ${visibleSql('ci')} AND qp.language_code IN (${LOCALE_SQL}) AND ci.attribution_status='verified' AND TRIM(COALESCE(ci.author_name,''))<>'' ORDER BY qp.language_code,ci.author_name`).bind(PROJECT_ID).all()).results||[];
}
