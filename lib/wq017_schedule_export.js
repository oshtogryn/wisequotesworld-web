export async function wq017ScheduleExport(request,env){
  ...
  try{
    const [item,versions,outputs,pages,media,reviews,approvals]=await Promise.all([
      ...
      q(`SELECT * FROM content_versions WHERE project_id='wisequotesworld' AND content_item_id='WQ017' ORDER BY language_code, version DESC`),
      ...
      q(`SELECT * FROM content_approvals WHERE project_id='wisequotesworld' AND content_item_id='WQ017' ORDER BY updated_at DESC`)
    ]);
