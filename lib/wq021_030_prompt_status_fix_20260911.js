...
async function rewriteWQ022Prompts(env,ts){
 ...
 for(const lang of SOCIAL){
  ...
  await env.DB.prepare(`UPDATE content_versions SET ai_prompt=?,editor_notes=COALESCE(editor_notes,'')||?,updated_at=? WHERE id=?`).bind(p,' Gemini-safe visual revision 2026-09-11: static museum sculpture only; no living/animated author depiction.',ts,row.id).run();
