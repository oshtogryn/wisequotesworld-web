...
function validDate(v){return typeof v==='string'&&v.trim()!==''&&Number.isFinite(Date.parse(v))}
...
async function duePublications(env){
 const rows=(await env.DB.prepare(`SELECT p.*,cv.content_id,cv.language_code FROM publications p JOIN content_versions cv ON cv.id=p.content_version_id WHERE p.status='scheduled' AND p.scheduled_at IS NOT NULL AND datetime(p.scheduled_at)<=datetime(?) ORDER BY datetime(p.scheduled_at) ASC LIMIT 100`).bind(now()).all()).results||[];
...
async function createPublication(request,env){
 ...
 const status=['draft','scheduled'].includes(String(b.status||''))?String(b.status):'draft';
 if(status==='scheduled'&&!b.scheduled_at)return json({ok:false,error:'scheduled_at required for scheduled status'},400);
 if(b.scheduled_at&&!validDate(b.scheduled_at))return json({ok:false,error:'scheduled_at must be a valid ISO/date-time value'},400);
...
async function readbackPublication(request,env,id){
 ...
 if(b.published_at&&!validDate(b.published_at))return json({ok:false,error:'published_at must be a valid ISO/date-time value'},400);
 try{
...
async function addAnalytics(request,env,id){
 ...
 const captured=b.captured_at||now();if(!validDate(captured))return json({ok:false,error:'captured_at must be a valid ISO/date-time value'},400);
...
async function analyticsSummary(env,url){
 ...
 const rows=(await env.DB.prepare(`
  WITH ranked AS (
    SELECT a.*,
           ROW_NUMBER() OVER (PARTITION BY a.publication_id ORDER BY datetime(a.captured_at) DESC,a.id DESC) AS rn
    FROM analytics_snapshots a
    WHERE a.project_id=? AND datetime(a.captured_at)>=datetime('now',?)
  )
  SELECT platform_code,
         ...
         SUM(COALESCE(watch_time_seconds,0)) AS watch_time_seconds
  FROM ranked
  WHERE rn=1
  GROUP BY platform_code
  ORDER BY platform_code
 `).bind(PROJECT_ID,`-${days} days`).all()).results||[];
 return json({ok:true,days,method:'latest_snapshot_per_publication',platforms:rows});
}
