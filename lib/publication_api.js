...
export async function publicationApi(request,env){
 ...
 // Legacy public diagnostic path is intentionally retired. Prompt-audit readback is
 // operational data and may be D1-heavy, so it must only run behind Access.
 if(url.pathname==='/prompt-audit-readback-20260914.json')return json({ok:false,error:'gone'},410);
 if(url.pathname==='/ops/readback/prompt-audit-20260914.json'&&request.method==='GET'){
