...
// `scheduled` and `published` are evidence-backed system states. They must never
// be set by a generic manual status control: scheduled requires Metricool Planner
// readback, published requires actual publication/readback.
const MANUAL_STATUSES=['idea','source_check','quote_ready','localized','native_language_qa','prompt_ready','copy_ready','website_ready','pinterest_ready','media_pending','media_ready','approved','skipped'];
const EVIDENCE_ONLY_STATUSES=new Set(['scheduled','published']);
...
async function listTopics(env){
 ...
 FROM content_items ci WHERE ci.project_id=? ORDER BY COALESCE(ci.sequence_no,0) ASC,ci.created_at ASC LIMIT 150`).bind(PROJECT).all()).results||[];
...
async function setStatus(env,id,b){
 ...
 if(EVIDENCE_ONLY_STATUSES.has(to))return json({ok:false,error:`${to} is evidence-backed and cannot be set manually`,required:to==='scheduled'?'Metricool Planner readback':'actual publication readback'},409);
 if(!MANUAL_STATUSES.includes(to))return json({ok:false,error:'unsupported status',allowed:MANUAL_STATUSES},400);
...
export async function adminTopicsApi(request,env){
 ...
 if(url.pathname==='/api/admin/topics'&&request.method==='GET')return json({ok:true,statuses:MANUAL_STATUSES,evidence_only_statuses:[...EVIDENCE_ONLY_STATUSES],items:await listTopics(env)});
