import {WQ017_SITE} from './wq017_site_payload.js';
import {applySitePackage} from './site_package_apply.js';
...
const WQ016={
  ...
  localizations:{
    uk:'«Лиш боротись значить жить…» — Іван Франко',ru:'«Лишь бороться — значит жить…» — Иван Франко',pl:'„Tylko walczyć — znaczy żyć…” — Iwan Franko',en:'“Only to struggle is to live…” — Ivan Franko',sv:'”Att kämpa är att leva…” — Ivan Franko',de:'„Nur kämpfen heißt leben…“ — Iwan Franko',es:'«Solo luchar significa vivir…» — Iván Frankó',fr:'«Lutter, c’est vivre…» — Ivan Franko',it:'«Solo lottare significa vivere…» — Ivan Franko',pt:'«Só lutar significa viver…» — Ivan Franko',id:'“Hanya dengan berjuang kita benar-benar hidup…” — Ivan Franko',tr:'“Yalnızca mücadele etmek, yaşamak demektir…” — Ivan Franko',ar:'«أن تكافح هو أن تعيش حقًا…» — إيفان فرانكو'
  }
};
const CANDIDATES={
  WQ017:{author:'Abraham Lincoln',theme:'character / responsibility',candidate:'“Let us have faith that right makes might.”',work:'Cooper Union Address',source:'https://www.nps.gov/liho/learn/historyculture/cooperunionaddress.htm',status:'source_check',risk:'source verified; website package prepared'},
...
export async function prepareFuturePreprod(env){
  ...
  try{
    ...
    if(!item)return{ok:false,stage,error:'WQ016 backlog item missing'};
    stage='evidence';
    const oldEvidence=await env.DB.prepare(`SELECT id FROM quote_source_evidence WHERE content_item_id=? LIMIT 1`).bind(WQ016.id).first();
    if(!oldEvidence)await env.DB.prepare(`INSERT INTO quote_source_evidence(content_item_id,source_type,source_title,source_url,source_locator,original_text,original_language,verified,verification_notes,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)`).bind(WQ016.id,'primary_text','Ivan Franko, Vivere memento!',WQ016.source_url,WQ016.source_locator,WQ016.original,'uk',1,'Primary text verified; line appears in final stanza.',ts).run();
    stage='update_wq016';
    await env.DB.prepare(`UPDATE content_items SET status='localized',quote_type='verbatim',original_quote=?,original_language='uk',author_name=?,source_name=?,source_url=?,source_work=?,attribution_status='verified',facts_verified=1,source_verified_at=?,source_verification_notes=?,notes=?,updated_at=? WHERE project_id=? AND id=?`).bind(WQ016.original,WQ016.author,WQ016.author_uk,WQ016.source_url,WQ016.work,ts,'Verified against Ivan Franko text “Vivere memento!”, final stanza, dated 14 October 1883. Quote-length gate PASS; all localized versions are short.','Pre-production only. WQ016 intentionally paused because author-name pronunciation QA is unresolved; do not publish.',ts,PROJECT,WQ016.id).run();
    ...
    stage='wq017_site';
    const wq017_site=await applySitePackage(env,WQ017_SITE);
    if(!wq017_site.ok)return{ok:false,stage,error:'WQ017 website publication failed',wq017_site};
    stage='readback';
    ...
    const wq017=await env.DB.prepare(`SELECT id,status,quote_type,author_name,source_work,attribution_status,facts_verified FROM content_items WHERE project_id=? AND id='WQ017'`).bind(PROJECT).first();
    const wq017_pages=(await env.DB.prepare(`SELECT language_code,slug,status,published_at FROM quote_pages WHERE project_id=? AND content_item_id='WQ017' ORDER BY language_code`).bind(PROJECT).all()).results||[];
    const next=(await env.DB.prepare(`SELECT id,status,source_name,source_work,notes FROM content_items WHERE project_id=? AND sequence_no BETWEEN 18 AND 25 ORDER BY sequence_no`).bind(PROJECT).all()).results||[];
    return{ok:true,stage:'done',wq016,localizations:versions.length,versions,wq017,wq017_site,wq017_pages,next};
