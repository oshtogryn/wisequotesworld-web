import {ensurePromptAuditStep,readPromptAudit} from './prompt_audit_023_030_10langs_20260914.js';
...
export async function ensureWebsiteSchedule(env){
  ...
  // One-time final prompt audit. The step function is idempotent via D1
  // system_migrations. Run enough iterations to finish WQ023-WQ030 in one request.
  for(let i=0;i<9;i++){
    const r=await ensurePromptAuditStep(env);
    if(r?.done)break;
  }
  const audit=await readPromptAudit(env);
  if(!audit?.ok)throw new Error(`final prompt audit incomplete: ${audit?.video_prompts||0}/80 video, ${audit?.pinterest_prompts||0}/80 Pinterest`);
  // D1 prepare/run is deliberately used one statement at a time here.
  // This avoids D1 exec() statement-splitting edge cases that can surface as
  // `CREATE TABLE ... incomplete input` in production.
  await env.DB.prepare(`CREATE TABLE IF NOT EXISTS website_publication_schedule (
  ...
  await env.DB.prepare(`CREATE INDEX IF NOT EXISTS idx_website_publication_schedule_due
    ON website_publication_schedule(project_id,status,scheduled_for)`).run();
}
