...
export async function seedFutureTopics(env){
  ...
  for(const [id,seq,category,author,theme] of FUTURE_TOPICS){
    ...
    if(!existing){
      ...
      await env.DB.prepare(`UPDATE content_items SET sequence_no=?,category=?,canonical_title=?,source_name=?,notes=?,updated_at=? WHERE project_id=? AND id=?`).bind(seq,category,title,author,notes,ts,PROJECT,id).run();
      updated.push(id);
