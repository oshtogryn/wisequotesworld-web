...
async function applyConfirmedOperationalState(env){
  ...
  await env.DB.prepare(`UPDATE content_items SET status='media_ready' WHERE project_id=? AND id='WQ011' AND status NOT IN ('scheduled','published')`).bind(PROJECT_ID).run();
...
export async function readCanonicalRules(env){
 ...
 return {ok:true,expected:keys.length,present:keys.filter(k=>latest[k]).length,missing:keys.filter(k=>!latest[k]),rules:keys.map(k=>latest[k]||{rule_key:k,missing:true})};
}
