...
async function upsertCanonicalRules(env){
 ...
 for(const [group,key,value] of RULES){
   ...
   if(existing?.id){
     await env.DB.prepare(`UPDATE rules SET rule_group=?,rule_value=?,notes='Canonical runtime rule synchronized from code/MASTER_RULES v4.12 + latest explicit decisions.',mandatory=1,status='approved',effective_from=? WHERE id=?`).bind(group,value,t,existing.id).run();
   }else{
     await env.DB.prepare(`INSERT INTO rules(project_id,scope_type,language_code,platform_code,rule_group,rule_key,rule_value,notes,mandatory,status,version,effective_from) VALUES(?, 'project', NULL, NULL, ?, ?, ?, 'Canonical runtime rule synchronized from code/MASTER_RULES v4.12 + latest explicit decisions.', 1, 'approved', 1, ?)`).bind(PROJECT_ID,group,key,value,t).run();
   }
 }
}
