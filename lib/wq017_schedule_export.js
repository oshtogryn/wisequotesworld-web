...
async function exportPack(env,id){
  const item=await env.DB.prepare(`SELECT id,status,title,author_name,source_name,source_url,original_text,attribution_status,quote_type FROM content_items WHERE project_id='wisequotesworld' AND id=?`).bind(id).first();
