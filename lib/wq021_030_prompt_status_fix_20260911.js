...
async function markWQ021Published(env,ts){
 ...
 if(cur.status!=='published'){
  ...
  await env.DB.prepare(`INSERT INTO editorial_status_history(project_id,content_item_id,from_status,to_status,notes,actor,created_at) VALUES(?,?,?,?,?,?,?)`).bind(PROJECT,'WQ021',cur.status,'published','User confirmed actual publication on 2026-09-10','user-confirmed-publication',ts).run();
 }
...
async function rewriteWQ022Prompts(env){
 const changed=[];
 for(const lang of SOCIAL){
  ...
  await env.DB.prepare(`UPDATE content_versions SET ai_prompt=? WHERE id=?`).bind(p,row.id).run();
...
async function verify022030(env){
 ...
 const structural=rows.length===9&&rows.every(r=>Number(r.published_pages||0)===13&&Number(r.native_qa||0)>=13&&Number(r.video_prompts||0)===8&&Number(r.pinterest_prompts||0)===8&&Number(r.outputs||0)>=88);
 return{ok:structural&&promptSafe,count:rows.length,rows,wq022_prompt_safety:{ok:promptSafe,count:promptRows.length,languages:promptRows.map(x=>x.language_code)}};
}
...
export async function applyWQ021030Fix(env){
 ...
 const prompts=await rewriteWQ022Prompts(env);
