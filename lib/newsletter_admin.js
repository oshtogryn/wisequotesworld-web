import {latestDigestArticles,renderDigest} from './newsletter_digest_visible.js';
...
async function ensure(env){
  ...
  if(!row)throw new Error('newsletter_schema_not_migrated');
}
...
async function recoverStaleReservations(env,at=new Date()){
  const cutoff=new Date(at.getTime()-2*60*60*1000).toISOString();
  const r=await env.DB.prepare(`UPDATE newsletter_deliveries
    SET status='failed',error='stale_reservation_recovered'
    WHERE project_id=? AND status='reserved' AND datetime(created_at)<datetime(?)`).bind(PROJECT_ID,cutoff).run();
  return Number(r?.meta?.changes||0);
}

async function sentToday(env,at=new Date()){
  const start=new Date(Date.UTC(at.getUTCFullYear(),at.getUTCMonth(),at.getUTCDate())).toISOString();
  const row=await env.DB.prepare(`SELECT COUNT(*) n FROM newsletter_deliveries
    WHERE project_id=? AND status='sent' AND datetime(sent_at)>=datetime(?)`).bind(PROJECT_ID,start).first();
...
async function reserve(env,sub,key){
  try{
    const r=await env.DB.prepare(`INSERT INTO newsletter_deliveries(project_id,subscriber_id,cycle_key,language_code,email_masked,status,created_at)
      VALUES(?,?,?,?,?,'reserved',?)
      ON CONFLICT(project_id,subscriber_id,cycle_key) DO UPDATE SET
      ...
      .bind(PROJECT_ID,sub.subscriber_id,key,sub.language_code,maskEmail(sub.email),nowIso()).run();
...
export async function runNewsletterBatch(env,{force=false}={}){
  ...
  const recovered=await recoverStaleReservations(env,now);
  const dailyCap=Math.max(1,Math.min(Number(s.daily_cap)||300,300));
  const alreadySent=await sentToday(env,now);
  const remaining=Math.max(0,dailyCap-alreadySent);
  if(remaining<=0)return {ok:true,skipped:'daily_cap_reached',daily_cap:dailyCap,sent_today:alreadySent,recovered};

  const cap=Math.max(1,Math.min(Number(s.execution_cap)||40,remaining,100));
  ...
  for(const sub of rows){
    ...
    try{
      ...
      if(articles.length<3)throw new Error('not_enough_visible_published_articles');
  ...
  await env.DB.prepare('UPDATE newsletter_settings SET last_run_at=?,updated_at=? WHERE project_id=?').bind(finishedAt,finishedAt,PROJECT_ID).run();
  return {ok:failed===0,cycle_key:key,selected:rows.length,sent,failed,skipped,recovered,daily_cap:dailyCap,sent_today:alreadySent+sent,remaining_today:Math.max(0,remaining-sent),errors:errors.slice(0,10)};
}
...
export async function newsletterAdminApi(request,env){
  ...
  try{
    ...
    if(u.pathname==='/api/admin/newsletter/run-scheduled'&&request.method==='POST')return json(await runNewsletterBatch(env,{force:false}));
