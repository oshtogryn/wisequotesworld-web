...
WQ018:{slug:'nietzsche-why-to-live',quotePatch:{
 ...
 ru:['«Тот, у кого есть своё «зачем» жить, выдержит почти любые обстоятельства». — Фридрих Ницше','«Тот, кому есть ради чего жить, выдержит почти любое «как».» — Фридрих Ницше'],
 ...
 en:['“He who has a why to live can bear almost any how.” — Friedrich Nietzsche','“He who has a why to live for can bear almost any how.” — Friedrich Nietzsche'],
 sv:['”Den som har ett varför att leva för kan uthärda nästan vilka omständigheter som helst.” — Friedrich Nietzsche','”Den som har ett varför att leva för kan uthärda nästan vad som helst.” — Friedrich Nietzsche']
}},
...
WQ022:{slug:'einstein-imagination-knowledge',quotePatch:{
 sv:['”Fantasin är viktigare än kunskapen.” — Albert Einstein','”Fantasi är viktigare än kunskap.” — Albert Einstein']
}},
WQ023:{slug:'lesya-ukrainka-hope-live'},
WQ024:{slug:'kennedy-ask-what-you-can-do'},
WQ025:{slug:'tagore-mind-without-fear',quotePatch:{
 uk:['«Де розум вільний від страху, а голова тримається високо». — Рабіндранат Тагор','«Де розум вільний від страху, а голова гордо піднята». — Рабіндранат Тагор'],
 ru:['«Где разум свободен от страха, а голова держится высоко». — Рабиндранат Тагор','«Где разум свободен от страха, а голова высоко поднята». — Рабиндранат Тагор'],
 pl:['„Gdzie umysł jest wolny od lęku, a głowa trzymana wysoko.” — Rabindranath Tagore','„Gdzie umysł jest wolny od lęku, a głowa uniesiona wysoko.” — Rabindranath Tagore'],
 sv:['”Där sinnet är fritt från rädsla och huvudet hålls högt.” — Rabindranath Tagore','”Där sinnet är fritt från rädsla och huvudet bärs högt.” — Rabindranath Tagore'],
 es:['«Donde la mente está libre de miedo y la cabeza se mantiene erguida». — Rabindranath Tagore','«Donde la mente está libre de miedo y se lleva la cabeza bien alta». — Rabindranath Tagore'],
 fr:['«Là où l’esprit est sans peur et où la tête se tient haute.» — Rabindranath Tagore','«Là où l’esprit est sans peur et où l’on garde la tête haute.» — Rabindranath Tagore'],
 it:['«Dove la mente è senza paura e il capo è tenuto alto.» — Rabindranath Tagore','«Dove la mente è senza paura e si tiene alta la testa.» — Rabindranath Tagore'],
 pt:['“Onde a mente está sem medo e a cabeça se mantém erguida.” — Rabindranath Tagore','“Onde a mente não conhece o medo e a cabeça se mantém erguida.” — Rabindranath Tagore']
}},
WQ026:{slug:'montaigne-art-to-live',quotePatch:{
 en:['“My trade and my art is to live.” — Michel de Montaigne','“Living is my trade and my art.” — Michel de Montaigne'],
 es:['«Mi oficio y mi arte es vivir». — Michel de Montaigne','«Vivir es mi oficio y mi arte». — Michel de Montaigne'],
 it:['«Il mio mestiere e la mia arte è vivere.» — Michel de Montaigne','«Vivere è il mio mestiere e la mia arte.» — Michel de Montaigne'],
 pt:['“Meu ofício e minha arte são viver.” — Michel de Montaigne','“Viver é meu ofício e minha arte.” — Michel de Montaigne']
}}
};
...
function clampWords(text,maxChars){const words=String(text||'').split(/\s+/);let out='';for(const w of words){const n=out?out+' '+w:w;if(n.length>maxChars)break;out=n}const shortened=out&&out.length<String(text||'').length;out=out.replace(/[,:;\-–—]+$/,'').trim();return shortened?out+'…':out}
function compose(body,suffix,min,max){const ps=paras(body),chosen=[];let base=suffix;for(const p of ps){const candidate=[...chosen,p,base].join('\n\n');if(candidate.length<=max){chosen.push(p);if(candidate.length>=min)break}else break}let text=[...chosen,base].join('\n\n');if(text.length<min){const used=chosen.length,next=ps[used];if(next){const room=Math.max(0,max-text.length-3);const clipped=clampWords(next,room);if(clipped)text=[...chosen,clipped,base].join('\n\n')}}if(text.length>max)text=text.slice(0,max).trimEnd();return text}
...
export async function applyPreparedTopicQuality(env,id){
 ...
 for(const [lang,pair] of Object.entries(cfg.quotePatch||{})){
   ...
   if(v&&v.adapted_text===oldQ)await env.DB.prepare(`UPDATE content_versions SET adapted_text=?,voiceover_text=?,on_screen_text=?,ai_prompt=?,language_check_status='native_qa_pass',editor_notes=? WHERE id=?`).bind(newQ,newQ,newQ,replaceQuoteInPrompt(v.ai_prompt,oldQ,newQ),'Native QA correction after editorial reread; semantic fidelity preserved.',v.id).run();
