...
export async function readbackWQ014Prompts(env){
 ...
 const cur=await env.DB.prepare(`SELECT status FROM content_items WHERE project_id=? AND id='WQ020'`).bind(PROJECT_ID).first();
 if(cur&&cur.status!=='scheduled'){
   await env.DB.prepare(`UPDATE content_items SET status='scheduled',updated_at=? WHERE project_id=? AND id='WQ020'`).bind(ts,PROJECT_ID).run();
   try{await env.DB.prepare(`UPDATE content_versions SET status=CASE WHEN status='published' THEN status ELSE 'scheduled' END,updated_at=? WHERE content_id='WQ020'`).bind(ts).run()}catch{}
   await env.DB.prepare(`INSERT INTO editorial_status_history(project_id,content_item_id,from_status,to_status,notes,actor,created_at) VALUES(?,?,?,?,?,?,?)`).bind(PROJECT_ID,'WQ020',cur.status,'scheduled','Metricool Planner readback confirmed 64/64 scheduled for 2026-09-08; minimum 15-minute cross-locale spacing per platform verified.','ops-readback',ts).run();
 }
 const wq020=await env.DB.prepare(`SELECT id,status,updated_at FROM content_items WHERE project_id=? AND id='WQ020'`).bind(PROJECT_ID).first();
 const h=(await env.DB.prepare(`SELECT from_status,to_status,notes,actor,created_at FROM editorial_status_history WHERE project_id=? AND content_item_id='WQ020' ORDER BY created_at DESC LIMIT 1`).bind(PROJECT_ID).all()).results||[];
 ...
 return {ok:languages.every(x=>x.video_prompt&&x.pinterest_prompt)&&wq020?.status==='scheduled',languages,video_prompt_count:languages.filter(x=>x.video_prompt).length,pinterest_prompt_count:languages.filter(x=>x.pinterest_prompt).length,wq020,latest_history:h[0]||null};
}
