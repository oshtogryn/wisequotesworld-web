...
async function schemaStatus(db){
 ...
 return{tables,content_items_columns:ci,v2,v3,v4,v5,v6,v7,v8,ready:v2&&v3&&v4&&v5&&v6&&v7&&v8};
}
...
async function applyMigrations(request,env){
 ...
 const steps=[['v3','/db/schema_v3.sql'],['v4','/db/migration4_prepare_fr_and_cutover.sql'],['v5','/db/migration5_content_origin.sql'],['v6','/db/migration6_quote_pages_and_versions.sql'],['v7','/db/migration7_seed_required_outputs.sql'],['v8','/db/migration8_ai_generation.sql']];
 for(const [key,path] of steps){if(!s[key]){await applyFile(request,env,path);applied.push(key);s=await schemaStatus(env.DB)}}
...
async function createContent(request,env){
 ...
 if(type==='verbatim'&&!String(b.author_name||'').trim())return json({ok:false,error:'author required for verbatim quote'},400);
...
async function updateContent(request,env,id){
 ...
 const type=b.quote_type??cur.quote_type;let author=('author_name'in b)?b.author_name:cur.author_name;if(type==='adapted')author=null;if(type==='verbatim'&&!String(author||'').trim())return json({ok:false,error:'author required for verbatim quote'},400);
 await env.DB.prepare(`UPDATE content_items SET canonical_title=?,original_quote=?,source_text=?,original_language=?,author_name=?,author_source=?,source_work=?,source_date=?,source_url=?,quote_type=?,category=?,category_slug=?,attribution_status=?,notes=?,updated_at=? WHERE id=? AND project_id=?`).bind(b.title??cur.canonical_title,b.quote??cur.original_quote,b.quote??cur.source_text,b.original_language??cur.original_language,author,b.author_source??cur.author_source,b.source_work??cur.source_work,b.source_date??cur.source_date,b.source_url??cur.source_url,type,b.category_slug??cur.category,b.category_slug??cur.category_slug,type==='adapted'?'not_required':(b.attribution_status??cur.attribution_status),b.notes??cur.notes,now(),id,PROJECT_ID).run();
...
async function generatePinterest(request,env,id,lang){
 ...
 const b=await reqBody(request)||{};const author=item.author_name&&item.attribution_status==='verified'?item.author_name:null;
 ...
 try{
  ...
  try{
   ...
   return json({ok:true,job_id:jobId,media_id:mediaId,r2_key:key,qa_status:'pending',model:PIN_MODEL},201);
