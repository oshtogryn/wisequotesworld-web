...
async function realActiveSubscribers(env){if(!env?.DB)return [];const rows=await env.DB.prepare(`SELECT rowid AS subscriber_id,email,language_code,unsubscribe_token,updated_at FROM newsletter_subscribers WHERE project_id=? AND status='active' ORDER BY updated_at ASC LIMIT 200`).bind(PROJECT_ID).all();return (rows.results||[]).filter(x=>!isDummyEmail(x.email))}
async function firstRealActiveSubscriber(env){const rows=await realActiveSubscribers(env);return rows[0]||null}
async function subscriberById(env,id){const n=Number(id);if(!Number.isInteger(n)||n<=0||!env?.DB)return null;const row=await env.DB.prepare(`SELECT rowid AS subscriber_id,email,language_code,unsubscribe_token,updated_at FROM newsletter_subscribers WHERE project_id=? AND status='active' AND rowid=? LIMIT 1`).bind(PROJECT_ID,n).first();return row&&!isDummyEmail(row.email)?row:null}
async function selectedSubscriber(env,id){return id?await subscriberById(env,id):await firstRealActiveSubscriber(env)}
...
export async function brevoNewsletterApi(request,env){
 ...
 if(!u.pathname.startsWith('/ops/'))return null;
 // Every /ops route must pass the same canonical admin middleware. The
 // top-level Worker also gates /ops, but this module remains fail-closed when
 ...
 if(u.pathname==='/ops/readback/brevo-health.json'&&request.method==='GET'){
  ...
  try{const r=await brevoFetch(env,'/account',{method:'GET'});await r.text();return json({ok:r.ok,configured:true,provider:'brevo',provider_status:r.status,sender_email:env.BREVO_SENDER_EMAIL,sender_name:env.BREVO_SENDER_NAME,delivery_mode:'digest_test_ready_scheduler_disabled',cadence_days:14,daily_cap:300},r.ok?200:502)}catch(e){return json({ok:false,configured:true,provider:'brevo',error:String(e?.message||e)},502)}
 }
 if(u.pathname==='/ops/readback/brevo-test-subscribers'&&request.method==='GET'){
  const rows=await realActiveSubscribers(env);
  return json({ok:true,count:rows.length,subscribers:rows.map(x=>({subscriber_id:x.subscriber_id,email_masked:maskEmail(x.email),language:x.language_code,updated_at:x.updated_at||null}))});
 }
 if(u.pathname==='/ops/readback/brevo-test-status'&&request.method==='GET'){
  const sub=await selectedSubscriber(env,u.searchParams.get('subscriber_id'));if(!sub)return json({ok:false,error:'real_active_subscriber_not_found'},404);
  try{const q=new URLSearchParams({email:String(sub.email),days:'1',limit:'50',sort:'desc'});const r=await brevoFetch(env,`/smtp/statistics/events?${q.toString()}`,{method:'GET'});const body=await r.text();let data={};try{data=body?JSON.parse(body):{}}catch{data={}}if(!r.ok)return json({ok:false,error:`brevo_events_${r.status}`},502);const exact=(data.events||[]).filter(x=>String(x.email||'').toLowerCase()===String(sub.email||'').toLowerCase()).slice(0,20).map(x=>({event:x.event||null,date:x.date||null,reason:x.reason||null,messageId:x.messageId||null,from:x.from||null}));return json({ok:true,subscriber_id:sub.subscriber_id,recipient_masked:maskEmail(sub.email),language:sub.language_code,event_count:exact.length,events:exact})}catch(e){return json({ok:false,error:String(e?.message||e)},502)}
 }
 if(u.pathname==='/ops/run/brevo-digest-test'&&request.method==='POST'){
  ...
  const sub=await selectedSubscriber(env,data.subscriber_id);if(!sub)return json({ok:false,error:'real_active_subscriber_not_found'},404);
  try{
   const articles=await latestDigestArticles(env,sub.language_code,5);
   if(articles.length<3)return json({ok:false,error:'not_enough_published_articles',article_count:articles.length},409);
   ...
   return json({ok:true,provider:'brevo',subscriber_id:sub.subscriber_id,recipient_masked:maskEmail(sub.email),language:digest.locale,article_count:articles.length,subject:digest.subject,scheduler_enabled:false,...sent});
 ...
 if(u.pathname==='/ops/run/brevo-test'&&request.method==='POST'){
  ...
  const sub=email?await env.DB.prepare(`SELECT rowid AS subscriber_id,email,language_code,unsubscribe_token FROM newsletter_subscribers WHERE project_id=? AND email=? AND status='active' ORDER BY updated_at DESC LIMIT 1`).bind(PROJECT_ID,email).first():await selectedSubscriber(env,data.subscriber_id);
  if(!sub||isDummyEmail(sub.email))return json({ok:false,error:'real_active_subscriber_not_found'},404);
  ...
  try{const sent=await sendBrevoEmail(env,{to:recipient,subject,htmlContent,textContent:`${headline}\n\n${intro}\n\nhttps://wisequotesworld.com/${String(sub.language_code)}/\n\n${unsubText}: ${unsubscribe}`,replyTo:'wisequotesofworld@gmail.com'});return json({ok:true,provider:'brevo',subscriber_id:sub.subscriber_id,recipient_masked:maskEmail(recipient),language:sub.language_code,...sent})}catch(e){return json({ok:false,error:String(e?.message||e)},502)}
 }
 return null;
}
